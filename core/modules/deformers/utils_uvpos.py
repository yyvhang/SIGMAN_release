...
if __name__ == '__main__':
    ...
    objpath = "./template/dense_thuman.obj"
    ...
    uv_pos = torch.as_tensor(np.load('./template/init_uv_smplx_thu.npy')).unsqueeze(0)
    uv_pos = uv_pos * 2 - 1 
    coord_pos = torch.as_tensor(np.load('./template/init_pcd_smplx_thu.npy'))
    primpos_map = primposmesh.permute(0, 3, 1, 2)
    primuv_map = primuv.permute(0, 3, 1, 2)
    np.save('./template/init_posmap_smplx_thu.npy', primpos_map.numpy())
    np.save('./template/init_uvmap_smplx_thu.npy', primuv_map.numpy())