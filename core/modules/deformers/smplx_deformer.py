...
class SMPLXDeformer(torch.nn.Module):
    ...
    def __init__(self, gender) -> None:
        super().__init__()
        self.body_model = SMPLX('./core/modules/deformers/smplx/SMPLX', gender=gender, \
        ...
        self.body_model_45 = SMPLX('./core/modules/deformers/smplx/SMPLX', gender=gender, \
                                create_body_pose=False, \
                                create_betas=False, \
                                create_global_orient=False, \
                                create_transl=False,
        ...
        init_spdir = torch.as_tensor(np.load('./core/modules/deformers/template/init_spdir_smplx_thu.npy'))
        self.register_buffer('init_spdir', init_spdir, persistent=False)

        init_podir = torch.as_tensor(np.load('./core/modules/deformers/template/init_podir_smplx_thu.npy'))
        self.register_buffer('init_podir', init_podir, persistent=False)
        
        init_faces = torch.as_tensor(np.load('./core/modules/deformers/template/init_faces_smplx_thu.npy'))
        self.register_buffer('init_faces', init_faces.unsqueeze(0), persistent=False)

        init_lbs_weights = torch.as_tensor(np.load('./core/modules/deformers/template/init_lbsw_smplx_thu.npy'))
    ...
    def prepare_deformer(self, smpl_params=None, num_scenes=1, device=None):
        ...
        if smpl_params is None:
            ...
            elif smpl_params.shape[1] == 179: #Human
                hand_45 = True
                transl, global_orient, betas, pose, expression, left_hand_pose, right_hand_pose = torch.split(smpl_params, [3, 3, 10, 63, 10, 45, 45], dim=1)
                #expression = torch.zeros(num_scenes, 10).to(device).float()
                transl = torch.as_tensor([[0., 0.35, 0.],]).to(device).repeat(num_scenes, 1).float()
                global_orient = torch.as_tensor([[0., 0., 0.],]).to(device).repeat(num_scenes, 1).float()
                scale = torch.as_tensor([[1.,],]).to(device).repeat(num_scenes, 1).float()
                jaw_pose = torch.zeros(num_scenes, 3).to(device).float()
            ...
            elif smpl_params.shape[1] == 188: #Human
                hand_45 = True
                transl, global_orient, betas, pose, expression, left_hand_pose, right_hand_pose, jaw_pose, leye_pose, reye_pose = torch.split(smpl_params, [3, 3, 10, 63, 10, 45, 45, 3, 3, 3], dim=1)
                transl = torch.as_tensor([[0., 0.35, 0.],]).to(device).repeat(num_scenes, 1).float()
                global_orient = torch.as_tensor([[0., 0., 0.],]).to(device).repeat(num_scenes, 1).float()
                scale = torch.as_tensor([[1.,],]).to(device).repeat(num_scenes, 1).float()
                smpl_params = {
