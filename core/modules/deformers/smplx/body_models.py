...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        **kwargs
        ...
        self.pose_mean = self.pose_mean.to(full_pose.device)
        full_pose += self.pose_mean
        ...
        # vertices, joints, T = lbs(shape_components, full_pose, self.v_template,
        #                        shapedirs, self.posedirs,
        #                        self.J_regressor, self.parents,
        #                        self.lbs_weights, pose2rot=pose2rot,
        #                        )
        # vertices, joints_smpl, T_weighted, W, T = lbs(shape_components, full_pose, self.v_template,
        #                        shapedirs, self.posedirs,
        #                        self.J_regressor, self.parents,
        #                        self.lbs_weights, pose2rot=pose2rot,
        #                        )
        ...
        # joints_smpl = joints_smpl * (scale.reshape(-1,1,1))
        # joints = joints * (scale.reshape(-1,1,1))
        # vertices = vertices * (scale.reshape(-1,1,1))
        # A[..., :3,:3] = A[..., :3,:3] * (scale.reshape(-1,1,1,1))
        # T[..., :3,:3] = T[..., :3,:3] * (scale.reshape(-1,1,1,1))
