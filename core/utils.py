...
def orbit_camera_jitter(poses, strength=0.1, is_w2c=False):
    B = poses.shape[0]
    ...
    if is_w2c:
        ...
        new_poses[:, :3, :3] = poses[:, :3, :3] @ rot_inv
        new_poses[:, :3, 3:] = -new_poses[:, :3, :3] @ rot @ (-poses[:, :3, 3:])
    else:
        new_poses[:, :3, :3] = rot @ poses[:, :3, :3]
        new_poses[:, :3, 3:] = rot @ poses[:, :3, 3:]
