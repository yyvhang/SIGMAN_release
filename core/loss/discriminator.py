...
class NLayerDiscriminator2D(nn.Module):
    ...
    def __init__(self, input_nc=1, ndf=64, n_layers=3, use_actnorm=False):
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            nf_mult = min(2 ** n, 8)
            sequence += [
                nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=(kw, kw), stride=2, padding=padw, bias=use_bias),
                norm_layer(ndf * nf_mult),
                nn.LeakyReLU(0.2, True)
            ]
        ...
        nf_mult = min(2 ** n_layers, 8)
        sequence += [
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=(kw, kw), stride=1, padding=padw, bias=use_bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True)
        ]
    ...
    def forward(self, input):
        """Standard forward.
            input: (B, V, C, H, W)
        """
        B,C,V,H,W = input.shape
        input = rearrange(input, 'b c v h w -> (b v) c h w')
        return self.main(input)