...
class HGS_1M(Dataset):
    ...
    def __init__(self, opt: Options, training=True):
        ...
        self.items = np.load('./data/train_VAE.npy').tolist()
    ...
    def __getitem__(self, idx):
        ...
        vid_cnt = 0
        if self.training:
            vids = [30, 37, 45, 53, 65, 85] + np.random.permutation(range(0, 89)).tolist()  #final use
        else:
            vids = [30, 37, 45, 53, 65, 85, 0, 8, 82, 60] # eval during training
