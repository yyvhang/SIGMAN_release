...
class HGS_1M(Dataset):
    ...
    def __init__(self, opt: Options, training=True):
        ...
        self.items = np.load('./data/test.npy').tolist()
        intr = np.array([
            [
                1100.0,
                0.0,
    ...
    def __getitem__(self, idx):
        ...
        for vid in vids:
            ...
            #face_mask_path = os.path.join(uid, 'face_mask',f'{vid}.png')
            ...
            #face_masks.append(face_mask.squeeze(0))
        ...
        _, _, h, w = images[0:1].shape
        crop_size = 768
        max_offset = h - crop_size
        offset_h = random.randint(0, max_offset)
        offset_w = random.randint(0, max_offset)
        cropped = images[0:1, :, offset_h:offset_h+crop_size, offset_w:offset_w+crop_size]
        sapines_input = F.interpolate(cropped, size=(1024, 1024), mode='bilinear', align_corners=False) # [V, C, H, W]
